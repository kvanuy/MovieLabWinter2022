...
for(int x=0; x<centerX; x=x+1){
	for (int y=0; y<centerY; y=y+1){
		radx = (midx-x)*(midx-x);
		rady = (midy-y)*(midy-y);
		if ((radx+rady)> (radius*radius)){
			SetPixelR(image,x,y,0);
			SetPixelG(image,x,y,0);
			SetPixelB(image,x,y,0);
}
...
for (int x=centerX; x<(2*centerX);x=x+1){
	for (int y=0; y<centerY; y=y+1){
		 radx = (x- midx)*(x- midx);
                 rady = (midy-y)*(midy-y);
                if ((radx+rady)> (radius*radius)){
...
for (int x=0; x<centerX;x=x+1){
for (int y=centerY; y<(centerY*2); y=y+1){
  radx = (midx-x)*(midx-x);
                rady = (y-midy)*(y-midy);
...
for (int x = centerX; x<(centerX*2);x=x+1){
for (int y = centerY; y<(centerY*2); y=y+1){
		  radx = (x-midx)*(x-midx);
                 rady = (y-midy)*(y-midy);
                if ((radx+rady)> (radius*radius)){
